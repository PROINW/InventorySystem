...
function dailySales($year, $month)
{
  ...
  $db->query("SET lc_time_names = 'th_TH';");

  $sql  = "SELECT s.qty,";
  $sql .= " DATE_FORMAT(s.date, '%e %M %Y') AS date, p.name,";
  $sql .= " SUM(p.sale_price * s.qty) AS total_saleing_price";
  $sql .= " FROM sales s";
  $sql .= " LEFT JOIN products p ON s.product_id = p.id";
...
/*--------------------------------------------------------------*/
/* Function for Generate monthlySales report
/*--------------------------------------------------------------*/
function monthlySales($year){
  ...
  $sql .= " DATE_FORMAT(s.date, '%M %Y') AS date";
  $sql .= " FROM sales s";
  $sql .= " LEFT JOIN products p ON s.product_id = p.id";
  $sql .= " WHERE DATE_FORMAT(s.date, '%Y') = '{$year}'";
  $sql .= " GROUP BY DATE_FORMAT(s.date, '%Y-%m'), s.product_id";
  $sql .= " ORDER BY DATE_FORMAT(s.date, '%Y-%m') DESC";

  return find_by_sql($sql);
}
function search_product_table($search_term) {
  global $db;
